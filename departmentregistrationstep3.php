		...
		if(empty($teacherBirthCountryErr) and empty($fatherBirthCountryErr)and empty($motherBirthCountryErr)and empty($grandfatherBirthCountryFatherSideErr)and empty($grandfatherBirthCountryMotherSideErr)and empty($grandmotherBirthCountryMotherSideErr)and empty($grandmotherBirthCountryFatherSideErr)){
		  ...
		  if($religion == ""){
			  $religion="Null";
				...
				data::insertinto('`age_group`','`age`, `user_id`, `type`',"'$age' , '$email', 5 ");
			  
			  include 'connection.php';
				$allDataInserted ="no";
				$languageerror="no";
				if(!mysql_query("UPDATE deptadmin SET status='active', firstname='".$firstName."', lastname='".$lastName."', religion='".$religion."', gender='".$gender."', password='".$hashAndSaltedPassword."' WHERE deptemailid='".$email."' AND token='".$token."' AND status='pending'",$con)){ 
					$allDataInserted ="no";
					...
					for($i=0; $i<$howmanylanguages;$i++){			
						...
						if(!mysql_query("insert into deptadminlanguage(deptemailid, languagename, languagelevel) values('$email','$language','$languageLevel')",$con)){
								$allDataInserted ="no";
								$languageerror="yes";
								die("ERROR: Data not inserted".mysql_error());
								break;
				...
				if($allDataInserted == "no"){
					...
					mysql_query("delete from deptadmin where deptemailid='".$email."'",$con);
					
					mysql_query("insert into deptadmin(deptemailid, orgid, firstname, token, status) values('$email','$orgId','$firstName','$token','pending')",$con);
					
					
					
					echo '<script type="text/javascript">'; 
							echo 'alert("Department Registration UnSuccessfull. Please follow registration link again from the email.");'; 
						...
						session_destroy(); 
							...
							echo '<script type="text/javascript">'; 
							echo 'alert("Department Admin Registration Successfull.");'; 
							echo 'window.location.href = "deptthanks.php";';
							echo '</script>';
				}
                ...
                <div class="col-sm-12 text-center" style="background-color:#FFF; margin-bottom:30px; margin-top:2em; padding-bottom:30px;">
                       ...
                       <form action="" method="post" >
                            ...
                            <div class="form-group form-inline">
                             ...
                             <input type="text" class="form-control inputfield" style="margin-left:0.5%; width:93%;" id="tbp" name="tbp"  placeholder="Department Admin birthplace*" value="<?php echo $teacherBirthCountry; ?>" >                                                          
