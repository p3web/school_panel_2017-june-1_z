...
Map.setValue = function () {
    ...
    for (var i = 0; i < checked.length; i++) {
        formData.push(checked[i].value);
    }
    params = {act: 'get_map', formDoor: formData};


    ajax.sender_data_json_by_url_callback(Map.url, params, Map.CreateMapData);

};

/*checked or un checked All*/
Map.changeAll = function (elem) {
    var checked = document.querySelectorAll('.mapCheck');
    for (var i = 0; i < checked.length; i++) {
        if (elem.checked) {
            ...
            Map.AllChecked = false;
        }
    }
};

Map.CreateMapData = function (data) {
    // _____ for Map
    var MapDatas = [
        [
